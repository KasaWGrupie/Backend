...
) : ControllerBase
{
    ...
    {
        updateMoneyRequestStatusDto = updateMoneyRequestStatusDto with { RequestId = requestId };
        
        var command = new UpdateMoneyRequestStatusCommand(updateMoneyRequestStatusDto);
        var result = await mediator.Send(command);
